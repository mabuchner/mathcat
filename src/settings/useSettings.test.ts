...
describe('useSettings', () => {
  ...
  it('persists updates to localStorage and reflects them in state', () => {
    ...
    act(() => {
      result.current.setSettings({ countdownSeconds: 15, soundEnabled: false })
    })

    expect(result.current.settings.countdownSeconds).toBe(15)
    expect(result.current.settings.soundEnabled).toBe(false)

    const stored = JSON.parse(window.localStorage.getItem(STORAGE_KEY) ?? '{}')
    expect(stored.countdownSeconds).toBe(15)
